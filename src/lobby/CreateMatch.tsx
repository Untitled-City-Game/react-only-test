...
import { Button, Group, Paper, Radio, Select, Stack, TextInput } from "@mantine/core";
...
export default function CreateGame() {
	...
	const teamCards = teamOptions.map((item) => (
		<Radio.Card radius="md" value={item.value} key={item.value}>
			...
			<Group wrap="nowrap" align="center">
				<Radio.Indicator color={item.value} size="lg" />
				<div>
	...
	const createGameForm = useForm({
		...
		validate : {
			...
			GameName: hasLength({min: 2, max: 20}, "Game name must be between 2 and 20 characters"),
