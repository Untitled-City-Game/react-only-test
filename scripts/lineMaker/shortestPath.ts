...
// index.ts
function ShortestPath(start: Coord, end: Coord, options?: {
  ...
  let diagonal = options.diagonal || false;
  ...
  const collection : FeatureCollection<Polygon | Point> = obstacles;
  ...
  const graph = new Graph(matrix, { diagonal });
